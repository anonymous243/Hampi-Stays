  ...
  return (
    <div className="min-h-screen bg-sand-50 pt-20">
      ...
      <div className="container mx-auto px-4 md:px-6 py-6">
        <Link
          to="/resorts"
          className="inline-flex items-center gap-2 text-stone-600 hover:text-forest-800 font-semibold text-sm transition-colors group"
        >
          ...
          {images.slice(1, 5).map((img, i) => (
            ...
            >
              <img
                ...
                className={`w-full h-full object-cover hover:scale-105 transition-transform duration-700 ${galleryIdx === i + 1 ? "ring-4 ring-terracotta-500" : ""}`}
              />
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              ...
              <h1 className="text-4xl md:text-5xl font-serif font-bold text-forest-950 mb-3 leading-tight">
                {resort.name}
              </h1>

              <p className="text-lg text-terracotta-600 font-medium italic mb-4">{resort.tagline}</p>
              ...
              <div className="flex items-center gap-4 mb-8">
                <div className="flex items-center gap-1.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`w-5 h-5 ${i < Math.floor(resort.rating) ? "fill-terracotta-500 text-terracotta-500" : "text-stone-200"}`}
                    />
                  ))}
                </div>
            ...
            <section className="mb-10">
              <h2 className="text-2xl font-bold font-serif text-forest-950 mb-4">About this property</h2>
              <p className="text-stone-700 leading-relaxed text-lg">{resort.description}</p>
            </section>

            {/* Amenities */}
            <section className="mb-10">
              <h2 className="text-2xl font-bold font-serif text-forest-950 mb-5">What's included</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {resort.amenities.map((amenity) => (
                  ...
                  >
                    ...
                    <span className="text-stone-800 font-medium text-sm">{amenity}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            {/* Policies */}
            <section className="mb-10">
              <h2 className="text-2xl font-bold font-serif text-forest-950 mb-5">Property Rules</h2>
              <div className="bg-white rounded-3xl border border-stone-100 p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">Check-in</p>
                  <p className="font-semibold text-stone-900">{resort.policies.checkIn}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">Check-out</p>
                  <p className="font-semibold text-stone-900">{resort.policies.checkOut}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">Min. Stay</p>
                  <p className="font-semibold text-stone-900">{resort.policies.minNights} night{resort.policies.minNights !== 1 ? "s" : ""}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">Pets</p>
                ...
                <div className="md:col-span-2">
                  <p className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-1">Cancellation</p>
                  <p className="text-stone-700">{resort.policies.cancellation}</p>
                </div>
              </div>
            </section>
            ...
            {/* Map */}
            <section className="mb-10">
              <h2 className="text-2xl font-bold font-serif text-forest-950 mb-5">Location</h2>
              <div className="h-64 rounded-3xl overflow-hidden border border-stone-200">
